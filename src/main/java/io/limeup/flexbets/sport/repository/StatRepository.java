...
public interface StatRepository extends ExternalIdRepository<EventStat, Long> {
    ...
    @Query(value = """
		...
		future_event_map AS (
			SELECT DISTINCT ON (esp.sub_participant_id)
                esp.sub_participant_id,
            ...
            JOIN sport.event fe ON fe.id = esp.event_id AND fe.start_date > NOW()
            LEFT JOIN (
			SELECT esp.event_id, STRING_AGG(DISTINCT p.acronym, ':' ORDER BY p.acronym) AS event_acronyms
                	FROM sport.event_sub_participant esp
					JOIN sport.participant p ON p.id = esp.participant_id
					JOIN sport.event e ON e.id = esp.event_id
				WHERE e.start_date > NOW()
			...
			ON acronyms.event_id = fe.id
			ORDER BY esp.sub_participant_id, fe.start_date
        ),
        ...
        SELECT
            ...
            ae.event_acronyms AS event_participant_acronyms,
        ...
        FROM paged_players pp
            ...
            ORDER BY
                ...
                CASE WHEN :sortBy = 'event_time' AND :sortOrder = 'asc' THEN fem.future_event_start_date END ASC,
                CASE WHEN :sortBy = 'event_time' AND :sortOrder = 'desc' THEN fem.future_event_start_date END DESC,
                fem.future_event_start_date, ae.start_date DESC
        """, nativeQuery = true)
    ...
    @Query(value = """
    	...
    	SELECT DISTINCT ON (esp.sub_participant_id)
    		...
    		acronyms.event_acronyms AS future_event_acronyms
    	FROM sport.event_sub_participant esp
    	JOIN sport.event e ON e.id = esp.event_id AND e.start_date > NOW()
    	LEFT JOIN (
    		SELECT esp.event_id,
    			   STRING_AGG(DISTINCT p.acronym, ':' ORDER BY p.acronym) AS event_acronyms
    		FROM sport.event_sub_participant esp
    ...
    @Query(value = """ 
		WITH unique_participants AS (
			...
			JOIN sport.event_sub_participant esp ON esp.participant_id = p.id
			JOIN sport.event e ON e.id = esp.event_id
			WHERE e.start_date > NOW()
		...
		future_event_map AS (
			...
			JOIN sport.event_sub_participant esp ON esp.participant_id = p.id
			JOIN sport.event fe ON fe.id = esp.event_id AND fe.start_date > NOW()
			LEFT JOIN sport.event_sub_participant esp2 ON esp2.event_id = fe.id
			LEFT JOIN sport.participant p2 ON p2.id = esp2.participant_id
		...
		top_played_events AS (
			SELECT *
				FROM (
					...
					JOIN sport.event_sub_participant esp ON esp.participant_id = pp.participant_id
					JOIN sport.event e ON e.id = esp.event_id AND e.start_date < NOW()
					JOIN sport.event_sub_participant esp2 ON esp2.event_id = e.id
					JOIN sport.participant p2 ON p2.id = esp2.participant_id
					WHERE e.start_date < NOW()
