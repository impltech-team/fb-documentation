...
public interface IoEventNFLRepository extends JpaRepository<IoEventNFL, Long> {

    Optional<IoEventNFL> findByGameKey(String gameKey);

    List<IoEventNFL> findAllByDatetimeUtcBetween(LocalDateTime from, LocalDateTime to);
    ...
    @Query(value = """
            ...
            SELECT e.*
            FROM paged p
            JOIN sport.io_event_nfl e ON e.id = p.id
            ORDER BY
                CASE WHEN :sortBy = 'event_name' AND :sortOrder = 'asc' THEN e.home_team || ' vs ' || e.away_team END ASC,
                CASE WHEN :sortBy = 'event_name' AND :sortOrder = 'desc' THEN e.home_team || ' vs ' || e.away_team END DESC,
                CASE WHEN :sortBy = 'event_date' AND :sortOrder = 'asc' THEN e.datetime_utc END ASC,
